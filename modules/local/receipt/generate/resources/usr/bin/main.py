...
def generate_individual_receipt(processes, analysis_info, output_file):
    ...
    receipt = {
        ...
        "error_summary": [process.get('details', {}).get('error_details') or process.get('details', {}).get('error_message') for process in clean_processes if process.get('status') == 'FAILED'][0] if overall_status=='FAILED' else None,
